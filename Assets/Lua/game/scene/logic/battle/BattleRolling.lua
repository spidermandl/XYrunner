...
BattleRolling.tag = "BattleRolling"
BattleRolling.roleName = "UIRolling"
BattleRolling.RollingList={}						--存放滚动数字的列表

--添加指定label的滚动
function BattleRolling:AddRolling(labelName,labelType,targetLabel,from,to,allTime)
...
function BattleRolling:FindRolling(labelName)
...
function BattleRolling:FindRolling(rollingAttributes)
	if table.getn(self.RollingList)>0 then
		for i=1,table.getn(self.RollingList)do
...
--用来处理滚动的方法
function BattleRolling:Rolling(rollingAttributes)
...
--线性变化滚动
function BattleRolling:RollingLine(rollingAttributes)
...
--随机变化滚动
function BattleRolling:RollingRandom(rollingAttributes)
	local teStr=0
...
--Update中执行的循环方法
function BattleRolling:DoRolling()
	if table.getn(self.RollingList)>0 then
...
--停止指定labelName滚动
function BattleRolling:StopRolling(labelName)
	table.remove(self.RollingList,self:FindRolling(labelName))
end

--停止所有滚动
function BattleRolling:StopRollingAll()
	self.RollingList={}
end