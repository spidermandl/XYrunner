...
FloatModel = class()
FloatModel.Model3D = nil
FloatModel.rotationNum = 0
FloatModel.isRotating = false
FloatModel.StartTriggetPosition= Vector3(0,0,0) 					-----------3D触发器开始的位置
--------------------3D旋转操作----------------------------------------------------------------
-----------加载3d模型
function FloatModel:Load3DModel(targetName,url)
	self:Clear3DModel()
...
-----------清除3D模型
function FloatModel:Clear3DModel()
...
-----------启动旋转
function FloatModel:StartRotating( ... )
	self.isRotating=true
	self.StartTriggetPosition=UnityEngine.Input.mousePosition -- 以后改触屏
end

---------------结束旋转
function FloatModel:EndRotating( ... )
	self.isRotating=false
end

--------Update中调用
function FloatModel:Rotating3DModel()
