...
StoreResourceView = class ()
StoreResourceView.targetScene = nil --场景
StoreResourceView.storeTable = nil --该类型的商店道具

--初始化
function StoreResourceView:init(targetScene)
	self.targetScene = targetScene
	self:checkSend()
end
--检查是否需要请求数据
function StoreResourceView:checkSend()
	self.storeTable = self.targetScene.storeManager:GetStoreTabel(6)
	if self.storeTable == nil then
		self.targetScene.storeManager:SendStoreInfoReq(6)  --如果本地没有这个商城类型的数据就请求这个类型的商城数据
	else
		self:refresh()
	end
end
...
-- 刷新
function StoreResourceView:refresh()
	...
	for i = 1 , #self.storeTable do
		--if tonumber(storeTabels:GetData(i,"SHOP_TYPE")) == 6 then
