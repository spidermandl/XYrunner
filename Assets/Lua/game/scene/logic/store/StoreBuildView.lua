...
StoreBuildView = class ()
StoreBuildView.targetScene = nil --场景
StoreBuildView.storeTable = nil --该类型的商店道具

--初始化
function StoreBuildView:init(targetScene)
	self.targetScene = targetScene
	self:checkSend()
end
--检查是否需要请求数据
function StoreBuildView:checkSend()
	self.storeTable = self.targetScene.storeManager:GetStoreTabel(5)
	if self.storeTable == nil then
		self.targetScene.storeManager:SendStoreInfoReq(5)  --如果本地没有这个商城类型的数据就请求这个类型的商城数据
	else
		self:refresh()
	end
end
...
-- 刷新
function StoreBuildView:refresh()
	-- 加载可以出售的物品
	if self.storeTable == nil then
		return
	end
	...
	for i = 1 , #self.storeTable do
		--if tonumber(storeTabels:GetData(i,"SHOP_TYPE")) == 5 then
