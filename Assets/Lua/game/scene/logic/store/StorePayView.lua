...
StorePayView = class ()
StorePayView.targetScene = nil --场景
StorePayView.storeTable = nil --该类型的商店道具

--初始化
function StorePayView:init(targetScene)
	self.targetScene = targetScene
	self:checkSend()
end
--检查是否需要请求数据
function StorePayView:checkSend()
	self.storeTable = self.targetScene.storeManager:GetStoreTabel(7)
	if self.storeTable == nil then
		self.targetScene.storeManager:SendStoreInfoReq(7)  --如果本地没有这个商城类型的数据就请求这个类型的商城数据
	else
		self:refresh()
	end
end
...
function StorePayView:refresh()
	...
	for i = 1 , #self.storeTable do
		--if tonumber(storeTabels:GetData(i,"SHOP_TYPE")) == 7 then
		local id = self.storeTable[i].id --该道具的id
			...
			obj.transform.parent = self.targetScene.grid.transform
			obj.transform.localPosition = Vector3.zero
    		obj.transform.localScale = Vector3.one
			obj.name = id
			obj.transform:Find("Name"):GetComponent("UILabel").text = itemName
			local priceLabel = obj.transform:Find("StoreBuyBtn/Label")
			priceLabel.transform.localPosition = Vector3.zero
			priceLabel:GetComponent("UILabel").text = "￥"..money_num
			...
			print("money_type : "..money_type)
			print("self.targetScene:GetIconName(money_type) : "..self.targetScene:GetIconName(money_type))
			obj.transform:Find("StoreBuyBtn/Background 1"):GetComponent("UISprite").spriteName = self.targetScene:GetIconName(money_type)
			--限购折扣
