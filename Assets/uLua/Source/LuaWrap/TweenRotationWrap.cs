...
public class TweenRotationWrap
{
	public static void Register(IntPtr L)
	{
		...
		LuaField[] fields = new LuaField[]
		{
			...
			new LuaField("quaternionLerp", get_quaternionLerp, set_quaternionLerp),
	...
	static int get_quaternionLerp(IntPtr L)
	{
		...
		if (obj == null)
		{
			...
			if (types == LuaTypes.LUA_TTABLE)
			{
				LuaDLL.luaL_error(L, "unknown member name quaternionLerp");
			}
			else
			{
				LuaDLL.luaL_error(L, "attempt to index quaternionLerp on a nil value");
			}
		}

		LuaScriptMgr.Push(L, obj.quaternionLerp);
		return 1;
	}
	...
	static int set_quaternionLerp(IntPtr L)
	{
		...
		obj.quaternionLerp = LuaScriptMgr.GetBoolean(L, 3);
		return 0;
	}
