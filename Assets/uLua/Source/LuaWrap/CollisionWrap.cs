...
public class CollisionWrap
{
	public static void Register(IntPtr L)
	{
		...
		LuaField[] fields = new LuaField[]
		{
			...
			new LuaField("impulse", get_impulse, null),
		};
	...
	static int get_impulse(IntPtr L)
	{
		...
		if (obj == null)
		{
			...
			if (types == LuaTypes.LUA_TTABLE)
			{
				LuaDLL.luaL_error(L, "unknown member name impulse");
			}
			else
			{
				LuaDLL.luaL_error(L, "attempt to index impulse on a nil value");
			}
		}

		LuaScriptMgr.Push(L, obj.impulse);
		return 1;
	}
